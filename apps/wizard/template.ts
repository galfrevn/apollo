...
const generatorDirectory = join(import.meta.dir, 'generator');
const templateDirectory = join(import.meta.dir, 'template');

const generatorResult = Bun.spawnSync(['bun', join(generatorDirectory, 'build.ts')], {
  stdout: 'inherit',
  stderr: 'inherit',
});
...
cpSync(join(generatorDirectory, 'out'), templateDirectory, { recursive: true });
