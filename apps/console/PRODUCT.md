...
Name: "Apollo Console". Binding visual direction (user-revised 2026-08-12, superseding the earlier amber pixel-console look, which itself superseded the stacked-echo blueprint look): the Midday dashboard canon (github.com/midday-ai/midday) played straight, in full monochrome, dark only. Grayscale-only chrome — #0d0d0d ground, #0c0c0c cards, #1d1d1d hairline borders, #fafafa ink, #878787/#666666 grays — red reserved for failure; no accent color of any kind. Hedvig Letters Sans for all UI with Hedvig Letters Serif for the single overview greeting; square corners everywhere (rounded only on the switch and status dots); elevation by 1px border, never shadow; Material Design outline icons via a single Icons object; the four-square cluster mark survives as the brand, rendered monochrome. Sentence case; no tracked-uppercase labels. Do not copy Midday's logo or name — the grammar is theirs, the identity is Apollo's.
