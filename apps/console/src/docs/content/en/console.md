...
- **Memory.** Browse what Apollo remembers: raw memories, the consolidated owner-memory block, and the spoken lists. Trust what it knows instead of guessing.
- **Schedules.** View and cancel pending reminders and timers.
- **MCP servers.** Install and remove servers, complete their OAuth flows, and enable tools one by one with their safety levels — the management half of [Capabilities](/docs/capabilities).
- **Broadcast.** Speak through the desk from anywhere: type a phrase for Apollo to say in its own voice, or record audio that plays as-is. Broadcasts queue while the device is offline and deliver on reconnect.
...
The console authenticates with the dashboard secret, never the device one, and the split is deliberate. The device credential is compiled into firmware, so rotating it means reflashing or an OTA for every body you own. The dashboard credential lives in a browser tab and rotates freely with one `wrangler secret put`. Sharing one secret would mean a leaked browser tab compromises the device fleet.
