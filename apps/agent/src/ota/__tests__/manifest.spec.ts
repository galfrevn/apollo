...
describe('firmware manifest', () => {
  it('parses a valid manifest', async () => {
    ...
    await expect(readFirmwareManifest(mediaBlobStore)).resolves.toEqual({
      version: '2.5.0',
      key: 'firmware/apollo-2.5.0.bin',
    });
  });
  ...
  it('returns undefined when the manifest object is missing', async () => {
    await expect(
      readFirmwareManifest(createFakeMediaBlobStore()),
  ...
  it('returns undefined on unparseable JSON', async () => {
    ...
    await expect(readFirmwareManifest(mediaBlobStore)).resolves.toBeUndefined();
  });
