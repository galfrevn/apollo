...
apps/agent/src ────────┐
documentation/ ────────┤
documentation/skills/ ─┤──▶ tooling/starter/build.ts ──▶ tooling/starter/out/
...
- **wrangler.jsonc** — parsed (comment-tolerant, `tooling/starter/jsonc.ts`), then: `routes` removed, `workers_dev` forced on, the `containers` block, `Sandbox` Durable Object binding, and its migration entry removed. Re-enabling coding is the `apollo-tooling` skill's runbook.
...
- **Forbidden strings** (`tooling/starter/guard.ts`, rules in the manifest): the build fails if the output contains the owner's email, voice id, or `heyapollo` outside the explicitly allowed files. Adding a personal literal to `apps/agent` breaks the starter build the same day, forcing the genericization conversation at PR time.
- **Freshness anchors** (`tooling/starter/__tests__/anchors.spec.ts`, runs in `bun run check`): wire constants the skills quote (audio floor, chunk size, sample rates, segment length, confirm timeout, Vectorize dimensions, the `[[escucho]]` marker) are extracted from source and asserted against the skill files. Changing a constant without updating its skill fails the monorepo gate.
...
1. New binding or secret → the starter inherits it automatically; decide whether it is optional for strangers (declare it in `src/configuration/environment.d.ts` and degrade gracefully) or provisioned by `bootstrap` (add it to `tooling/starter/assets/scripts/bootstrap.ts` and the `apollo-setup` skill).
