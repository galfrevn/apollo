...
The server owns meaning; the device owns rendering. `apps/agent/src/persona/face.ts` maps the UI state machine (`apps/agent/src/session/machine.ts`) to a small emotion vocabulary, and every `ui_state` message carries that emotion plus the active speech mode's accent color. Firmware should not keep its own state→expression lookup table — it reads `emotion` and `accentColor` straight off the wire.
...
|-------|--------|---------|
| `emotion` | `resolveDeskFaceEmotion(uiState)` in `apps/agent/src/persona/face.ts` | Which expression to render |
| `accentColor` | Active speech mode's `accentColor` in `apps/agent/src/persona/catalog.ts` | Eye/UI tint, so personality is visible, not just audible |
...
(`apps/firmware/apollo-firmware`, a git submodule of this one), which has its own handbook and
